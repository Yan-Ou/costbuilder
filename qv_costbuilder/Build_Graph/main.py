        ...
        for key in result['Contents']:
            ...
            if not key['Key'].endswith('/'):
                ...
                client.download_file(
                    bucket, key['Key'], local_file_path)


client = boto3.client('s3')
dynamoDB = boto3.resource('dynamodb', region_name='ap-southeast-2')

download_dir(client, 'qrious-qv-data',
             'qv-data/costbuilder_inputs/', '.')
...
INPUTS_PATH = BASE_PATH  # lookups

# 1 House B1, breakdown.xlsx ...
PATH_TO_WORKBOOKS_XLSX = os.path.join(INPUTS_PATH, 'workbooks')
...
PATH_TO_PRICED_ELEMENTS_XLSX = os.path.join(
    INPUTS_PATH, 'priced_elements')  # priced_elements.xlsx
...
formula_lookup = pd.read_excel(
    os.path.join(INPUTS_PATH, 'formula_lookup.xlsx'))
...
for file in xlsx_files:
    print("Reading {}".format(file))
    stem = file.split('.')[0]
    priced_elements = pd.read_excel(
        os.path.join(PATH_TO_PRICED_ELEMENTS_XLSX, file))
...
for area in cities.keys():
    query = "CREATE ({}:City {{name: '{}'}});".format(
        cities[area].lower().replace(" ", "_"), cities[area])
    ...
    for index, row in dfs[file].iterrows():
        ...
        if str(row['id']) in unique_ids:
            error = "ERROR: id {} found in both {} and {}".format(row['id'],
                                                                  unique_ids_sources[unique_ids.index(
                                                                      row['id'])],
                                                                  file)
            errors.append(error)
            print(error)
    ...
    for file in dfs.keys():
        ...
        for index, row in dfs[file].iterrows():
            ...
            if USE_SQL:
                ...
                table = dynamoDB.Table(config.QV_PRICES)
                table.put_item(
                    Item={
                        'id': str(row['id']),
                        'name': str(row['name']).replace('"', '""'),
    ...
    for index, row in dfs[file].iterrows():
        ...
        if row['item_block'] == 'I':
            ...
            if USE_SQL:
                ...
                table = dynamoDB.Table(config.QV_BLOCKS)
                table.put_item(
                    Item={
                        'id': str(row['id']),
                        'uom': str(row['uom']),
                        'new_margin': str(row['new_margin']),
                        'new_discount': str(row['new_discount']),
                        'new_waste': str(row['new_waste']),
                        'qty': str(qty),
                        'factor': str(factor),
                        'include_in_lbr_hrs': str(row['include_in_lbr_hrs']),
                        'include_in_plant_costs': str(row['include_in_plant_costs']),
                        'include_in_material_costs': str(row['include_in_material_costs']),
                        'file': str(file)
                    }
                )
...
for file in xlsx_files:
    ...
    for index, row in df4.iterrows():
        ...
        if USE_SQL:
            ...
            table = dynamoDB.Table(config.QV_BLDGS)
            table.put_item(
                Item={
                    'id': str(row['id']),
                    'uom': str(row['uom']),
